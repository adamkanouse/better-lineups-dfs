...
from csv_parse import nfl_upload, nba_upload, mlb_upload, cfb_upload
...
def retrieve_players(args):
    if args.historical_date:
        ...
        all_players = []
        with open(args.salary_file, 'rU') as csv_file:
    ...
    if args.__dict__.get('use_average'):
        ...
        with open(args.projection_file, 'rU') as csvfile:
...
def _get_game(settings):
 ...
 #   game = cons.DRAFT_KINGS \
    if settings.game == 'draftkings':
        game = cons.DRAFT_KINGS
    elif settings.game == 'fanduel':
        game = cons.FAN_DUEL
    print (game)
    return game
...
def _set_player_ownership(all_players, args):
    if args.po_location and args.po:
        with open(args.po_location, 'rU') as csv_file:
            csv_data = csv.DictReader(csv_file)
            for row in csv_data:
                player = filter(
